...
|    | Back  | R16-R19, R31, R32          | Resistor         | 1kΩ                                     |         | R14-R17 control the brightness of the four bottom IO LEDs. In the dual-color lamps I bought, the red LED was significantly dimmer than the green LED, so I used 1/10th the resistance to get approximately consistent visual brightness. If you sourced your own LEDs, you might want to set up a simple breadboard circuit to dial in these values. The remaining 15 LEDs are PWM controlled, so the brightness is compensated for in software (can also be changed, but requires the code to be recompiled). |
|    | Back  | R14-R15, R20-R23, R30, R33 | Resistor         | 10kΩ                                    |         | R20-R21 and R22-R23 are matched pairs. Ideally, they would be hand-picked to match each other or they would be lower-tolerance resistors. But, RV1 and RV2 optionally let you compensate for any mismatches. |
